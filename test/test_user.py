...
from handlers.decorators import authenticate
...
class WebTest_User(unittest.TestCase):
    ...
    def testRegisterHandlerWeakPassword(self):
        ...
        self.assertEqual(response.status_int, constants.STATUS_BAD_REQUEST,
                         'Code constants.STATUS_BAD_REQUEST should arrive when calling with too weak password: ' + str(response.status_int))
    ...
    def testRegisterHandlerBadPassword(self):
        ...
        self.assertEqual(response.status_int, constants.STATUS_BAD_REQUEST,
                         'Code constants.STATUS_BAD_REQUEST should arrive when calling with invalid characters in password: ' + password + ' ' + str(response.status_int))
        password = '"a;"@a.hu'
        response = UserUtil.register_user(self.testapp, email, password)
        self.assertEqual(response.status_int, constants.STATUS_BAD_REQUEST,
                         'Code constants.STATUS_BAD_REQUEST should arrive when calling with invalid characters in password: ' + password + ' ' + str(response.status_int))
